...
app.post("/api/optimize", async (req, res) => {
	try {
		...
		console.log(
			`📁 폴더 수: ${files.filter((f) => f.mimeType === "application/vnd.google-apps.folder").length}개`
		)
...
async function simulateAIOptimization(files) {
	...
	const others = files.filter(
		(f) =>
			!folders.includes(f) &&
			!documents.includes(f) &&
			!spreadsheets.includes(f) &&
			!presentations.includes(f) &&
			!pdfs.includes(f) &&
			!images.includes(f)
	)
...
function analyzeChanges(originalFiles, optimizedFiles) {
	...
	originalFiles.forEach((originalFile) => {
		...
		if (
			optimizedFile &&
			JSON.stringify(originalFile.parents) !== JSON.stringify(optimizedFile.parents)
		) {
			movedFiles++
		}
	})
